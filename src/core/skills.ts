...
import { ensureDir, pathExists, removeIfExists } from "./fs.js";
import { syncBridge, cleanupManagedBridge } from "./bridge.js";
...
export async function syncSkills(args: {
  ...
  await syncBridge({
    enabled:
      (enabledIntegrations.includes("gemini") ||
        enabledIntegrations.includes("antigravity")) &&
      hasSkills,
    projectRoot,
...
async function cleanupLegacyAntigravityBridge(args: {
  ...
  const expectedRelative =
    path.relative(path.dirname(legacyBridgePath), paths.agentsSkillsDir) || ".";

  const removable = await cleanupManagedBridge(
    legacyBridgePath,
    expectedRelative,
  ...
  changed.push(
    path.relative(projectRoot, legacyBridgePath) || legacyBridgePath,
  );
  if (check) return;
