...
describe('reset command', () => {
  it('removes legacy Codex MCP tables while preserving trailing user settings', async () => {
    const projectRoot = await mkdtemp(path.join(os.tmpdir(), 'agents-reset-codex-legacy-'))

    try {
      const generated = [
        '# Generated by agents sync. Do not edit manually.',
        '',
        '[mcp_servers."old"]',
        'command = "old-server"',
        ''
      ...
      await writeFile(path.join(projectRoot, '.agents', 'generated', 'codex.config.toml'), `${generated}\n`)
      await writeFile(
      ...
      const cleaned = await readFile(path.join(projectRoot, '.codex', 'config.toml'), 'utf8')
      expect(cleaned).toContain('[features]')
      expect(cleaned).toContain('web_search = true')
      expect(cleaned).not.toContain('[mcp_servers."old"]')
    } finally {
  ...
  it('cleans local materialized files in local-only mode', async () => {
    ...
    try {
      ...
      await writeFile(path.join(projectRoot, '.codex', 'config.toml'), 'managed = true\n')
      await writeFile(path.join(projectRoot, '.codex', 'user-settings.toml'), 'keep\n')
      await writeFile(path.join(projectRoot, '.gemini', 'settings.json'), '{}\n')
      await writeFile(path.join(projectRoot, '.gemini', 'user-settings.json'), '{}\n')
      await writeFile(path.join(projectRoot, '.gemini', 'skills', '.agents_bridge'), 'managed-by-agents antigravity-flat-copy\n')
      await writeFile(path.join(projectRoot, 'CLAUDE.md'), '@AGENTS.md\n')
      await writeFile(path.join(projectRoot, '.claude', 'skills', 'guide.md'), 'x\n')
      await writeFile(path.join(projectRoot, '.claude', 'skills', '.agents_bridge'), 'managed-by-agents\n')
      await writeFile(path.join(projectRoot, '.windsurf', 'skills', 'sample.md'), 'x\n')
      await writeFile(path.join(projectRoot, '.windsurf', 'skills', '.agents_bridge'), 'managed-by-agents\n')
      ...
      expect(await exists(path.join(projectRoot, '.codex'))).toBe(true)
      expect(await exists(path.join(projectRoot, '.codex', 'user-settings.toml'))).toBe(true)
      expect(await exists(path.join(projectRoot, '.gemini'))).toBe(true)
      expect(await exists(path.join(projectRoot, '.gemini', 'user-settings.json'))).toBe(true)
      expect(await exists(path.join(projectRoot, '.gemini', 'skills'))).toBe(false)
      ...
      expect(await exists(path.join(projectRoot, '.opencode'))).toBe(true)
      expect(await exists(path.join(projectRoot, '.opencode', 'agent', 'sample.md'))).toBe(true)
